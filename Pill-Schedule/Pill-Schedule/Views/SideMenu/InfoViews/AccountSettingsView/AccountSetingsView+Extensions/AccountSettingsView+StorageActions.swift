...
extension AccountSettingsView {
    ...
    func uploadUserProfilePic() {
        ...
        guard let imageData = pic.jpegData(compressionQuality: 0.75) else {return}
    ...
    func retrieveUserProfilePic() {
        ...
        let downloadTask = profileImageReference.getData(maxSize: 4 * 1024 * 1024) { retrievedData, error in
            if let error = error {
                print(error.localizedDescription)
                return
            }
